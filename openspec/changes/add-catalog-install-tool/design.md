...
The MCP server will download the catalog TAR archive, index apps → service template versions, and expose the data through `k0.catalog.*` tools. Install requests will apply the ServiceTemplate (and required HelmRepository) into the management cluster using the existing dynamic client.
...
2. **Cache** – Maintain a cache manifest with ETag / commit sha. Only re-download when missing or `k0.catalog.list --refresh` is called.
...
4. **Serve list** – `k0.catalog.list` reads the index from cache; optional `app` filter returns matching slug. The handler never touches the network unless the cache is empty or refresh requested.
5. **Install** – `k0.catalog.install(app, template, version)` performs:
